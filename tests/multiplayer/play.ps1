...
$serviceRoot = Join-Path $repoRoot 'server'
if (-not (Test-Path -LiteralPath (Join-Path $serviceRoot 'node_modules'))) {
    throw 'Run npm ci in server before launching the local service.'
}
...
try {
    ...
    $service = Start-Process -FilePath $node -ArgumentList (Join-Path $serviceRoot 'main.ts') -WorkingDirectory $serviceRoot `
    ...
    while (-not (Test-Path -LiteralPath $serviceOut) -or -not ((Get-Content -LiteralPath $serviceOut -Raw -ErrorAction SilentlyContinue) -match '"event":"listening"')) {
        if ($service.HasExited) { throw "The online service exited early. See $serviceError" }
        if ((Get-Date) -gt $deadline) { throw "The online service did not become ready. See $serviceError" }
